...
app.post('/generate', async (req, res) => {
    ...
    const prompt = `
        ...
        **Core Instructions:**
        ...
        4.  **Resource Hub (NEW):** As the final card, always include a "#### Suggested Resources" card. Provide 2-3 clickable, real, and relevant links (e.g., to YouTube videos, educational websites like PhET, or news articles) that supplement the lesson. **You MUST format links as standard markdown: [Link Text](URL).**
        5.  **Output Formatting:** Structure the entire response as a single block of text. For each requested asset, start with a title formatted as "#### [Asset Title]". **Do not use bold asterisks (**) in the titles or content.** Separate each distinct part (e.g., the plan segment and each full asset) with the delimiter: "[CARD_BREAK]".
