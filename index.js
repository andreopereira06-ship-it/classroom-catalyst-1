...
app.post('/generate', async (req, res) => {
    ...
    // This is the new, advanced "meta-prompt" that guides the AI.
    const prompt = `
        ...
        **Core Instructions:**
        ...
        2.  **Local & Cultural Relevance (CRITICAL):** When a location is provided, you MUST weave in specific, familiar, and current references. Avoid generic examples.
            ...
            * **Fun Facts Rule:** "Fun facts" must be "familiar yet never heard before." Connect the topic to a surprising local or pop culture element. A generic fact is a failure.

        3.  **Output Formatting:** Structure your entire response as a single block of text. For each requested asset, start with a title formatted as "#### [Asset Title]" and then the content. Separate each complete asset (title + content) with the exact delimiter: "[CARD_BREAK]". Do not use this delimiter anywhere else.
